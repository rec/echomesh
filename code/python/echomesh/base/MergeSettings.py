...
class MergeSettings(object):
  ...
  def read(self):
    ...
    self.arg_settings = self._assignment_to_settings(self.settings, _ARGUMENT_ERROR)
  ...
  def save(self):
    ...
    self.arg_settings = Merge.difference_strict(self.arg_settings, self.changed)
    self.recalculate()
    return saved_files
  ...
  def _assignment_to_settings(self, args, error):
    ...
    for addr, value in split_args:
      try:
        GetPrefix.set_assignment(addr, value, base_settings, settings,
                                 unmapped_names=Merge.SETTINGS_EXCEPTIONS)
      except GetPrefix.PrefixException:
        raise Exception('Can\'t understand settings address "%s"' % addr)
      except Exception:
        raise Exception('Can\'t understand settings value "%s" in %s=%s' %
                        (value, addr, value))
    return settings
